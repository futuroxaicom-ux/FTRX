"""Sniper Bot v3 - Profit-focused strategy
Only buys tokens with high liquidity and upward trend.
Higher take profit targets to overcome slippage costs."""
import asyncio
import time
import logging
import base58
...
class SniperBot:
    ...
    async def _discover_new_pools(self):
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                # Token profiles - newest tokens
                try:
                    r = await client.get("https://api.dexscreener.com/token-profiles/latest/v1")
                    if r.status_code == 200:
                        profiles = r.json()
                        if isinstance(profiles, list):
                            for p in [x for x in profiles if x.get("chainId") == "solana"][:15]:
                ...
                for pair in pairs:
                    ...
                    # 1. HIGH LIQUIDITY only (reduces slippage)
                    liq = float(pair.get("liquidity", {}).get("usd", 0) or 0)
                    min_liq = self.config.get("min_liquidity_usd", 10000)
                    max_liq = self.config.get("max_liquidity_usd", 500000)
                    ...
                    # 4. TREND CHECK - only buy if price is going UP
                    price_change_5m = float(pair.get("priceChange", {}).get("m5", 0) or 0)
                    price_change_1h = float(pair.get("priceChange", {}).get("h1", 0) or 0)
                    min_change = self.config.get("min_price_change_5m", 5.0)
                    if price_change_5m < min_change and price_change_1h < min_change:
                        self.stats["tokens_rejected"] += 1
                        continue

                    # 5. VOLUME CHECK - must have real trading activity
                    vol_1h = float(pair.get("volume", {}).get("h1", 0) or 0)
                    min_vol = self.config.get("min_volume_1h_usd", 5000)
                    if vol_1h < min_vol:
                        self.stats["tokens_rejected"] += 1
                        continue
                    ...
                    self._log("NEW_POOL", "", liq, f"{token_symbol} | {dex} | Liq:${liq:.0f} | 5m:{price_change_5m:+.1f}% | Vol:${vol_1h:.0f} | Age:{age_min:.0f}m")
    ...
    async def _snipe_token(self, token_mint, token_symbol=""):
        ...
        # Estimate slippage cost BEFORE buying
        try:
            async with httpx.AsyncClient(timeout=8.0) as client:
                ...
                if abs(impact) > 5:
                    self._log("SKIP", pub, 0, f"Price impact too high: {impact:.1f}% for {token_symbol}")
                    self.stats["tokens_rejected"] += 1
                    return
        except Exception:
            pass
    ...
    async def _monitor_positions(self):
        for token_mint, pos in list(self._positions.items()):
            try:
                ...
                # Take profit - sell ALL at once (one slippage cost, not two)
                tp = self.config.get("take_profit_percent", 40)
                if change_pct >= tp:
                ...
                # Stop loss
                sl = self.config.get("stop_loss_percent", 20)
                if change_pct <= -sl:
                    await self._sell_position(token_mint, pos, f"STOP_LOSS {change_pct:.1f}%")
                    self.stats["stop_losses"] += 1
                    continue

                # Log position status every ~30s
                if int(time.time()) % 30 < self.config.get("check_interval", 8) + 1:
                    self._log("HOLD", pos["wallet"], pos["sol_spent"], f"{symbol} P/L:{change_pct:+.1f}% age:{age_min:.0f}m")
    ...
    async def _sell_position(self, token_mint, pos, reason):
        ...
        # Sell 95% with LOWER slippage (saves money on exit)
        sell_amount = int(raw_amount * 0.95)
        sell_slip = self.config.get("sell_slippage_bps", 800)
        kp = Keypair.from_bytes(base58.b58decode(wallet["private_key"]))
        result = await jupiter_swap(kp, token_mint, SOL_MINT, sell_amount, sell_slip)
        ...
        if result["success"]:
            ...
            self._log(reason.split()[0], wallet_pub, sol_out, f"{reason} {symbol} P/L:{profit:+.4f} SOL tx:{result.get('tx','')[:12]}")
    ...
    async def generate_wallets(self, count: int, prefix: str = "Sniper"):
        ...
        for i in range(count):
            ...
            doc = {
                "label": f"{prefix} #{existing + 1}", "public_key": str(kp.pubkey()),
